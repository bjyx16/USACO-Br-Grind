/*
- O(2n) basically
- to find max subarray sum: only 'worth it' to add negative numbers if sum of consecutive negative numbers doesn't cancel out whatever sum you have so far
if pos int, always add; if neg int, find the sum of neg numbers in consectutive subarray & if it cancels out the sum, it's not worth and start at next pos num
